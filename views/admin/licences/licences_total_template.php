...
<div class="row">
<?php if(count($licences_years) > 1 || isset($currencies)){ ?>
  <div class="col-md-12 simple-bootstrap-select mbot5">
   <?php if(isset($currencies)){ ?>
    <select class="selectpicker" data-width="auto" name="total_currency" onchange="init_licence_total();">
    ...
    <?php } ?>
      <?php
      if(count($licences_years) > 1){ ?>
      <select data-none-selected-text="<?php echo date('Y'); ?>" data-width="auto" class="selectpicker" multiple name="licences_total_years" onchange="init_licence_total();">
  ...
  foreach($totals as $key => $data){
    $class = licence_status_color_class($data['status']);
    $name = licence_status_by_id($data['status']);
    ?>
